    ...
    */
    ...
    public R page(@RequestParam Map<String, Object> params, HttpServletRequest request){
        ...
        if(false)
            ...
            FudaoyuanEntity fudaoyuanEntity = fudaoyuanService.selectById((Integer) request.getSession().getAttribute("userId"));
            if(fudaoyuanEntity != null){
                params.put("banjiTypes",fudaoyuanEntity.getBanjiTypes());
            }else{
                params.put("banjiTypes",-1);
            }
...
//            liuyan.setXueshengId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));
        ...
        logger.info("sql语句:"+queryWrapper.getSqlSegment());
        LiuyanEntity liuyanEntity = liuyanService.selectOne(queryWrapper);
        if("".equals(liuyan.getLiuyanFile()) || "null".equals(liuyan.getLiuyanFile())){
                liuyan.setLiuyanFile(null);
        }
...
//                            liuyanEntity.setLiuyanName(data.get(0));                    //留言名称 要改的
//                            liuyanEntity.setLiuyanTypes(Integer.valueOf(data.get(0)));   //留言类型 要改的
//                            liuyanEntity.setLiuyanFile(data.get(0));                    //留言附件 要改的
//                            liuyanEntity.setLiuyanContent("");//详情和图片
//                            liuyanEntity.setInsertTime(date);//时间
//                            liuyanEntity.setHuifuContent("");//详情和图片
